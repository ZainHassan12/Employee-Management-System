...
class CoAttendance extends StatefulWidget {
  const CoAttendance({Key? key}) : super(key: key);

  @override
  State<CoAttendance> createState() => _CoAttendanceState();
}

class _CoAttendanceState extends State<CoAttendance> {
  ...
  void getRecord() async{
    ...
    try{
      QuerySnapshot snap = await FirebaseFirestore.instance
          .collection("Manager").where('id', isEqualTo: User.username)
          .get();
  ...
  void getCount() async {
    try {
      QuerySnapshot snap2 = await FirebaseFirestore.instance
          .collection('Manager').doc(User.Id)
      ...
      snap2.docs.forEach((document) {
        ...
        if (DateFormat('MMMM yyyy').format(documentDate) == _month) {
          presentDays++;
        }
      });
  ...
  void updateMonth(DateTime selectedMonth) {
    setState(() {
      _month = DateFormat('MMMM yyyy').format(selectedMonth);
    });
    getCount();
  }
                ...
                children: [
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      if (date.isAfter(dayEndTime) && date.isBefore(nightEnd)){
                        formattedDate = DateFormat('dd MMMM yyyy').format(date.subtract(const Duration(days: 1)));
                        DateTime dateTime = DateTime.now().subtract(const Duration(days: 1));
                        Timestamp timestamp = Timestamp.fromDate(dateTime);

                        if(User.canCheck) {
                          QuerySnapshot snap = await FirebaseFirestore.instance
                              .collection("Manager").where(
                          ...
                          try {
                            ...
                            await FirebaseFirestore.instance.collection("Manager")
                                .doc(snap.docs[0].id).collection("Record").doc(
                                formattedDate)
                                .update({
                              'date': timestamp,
                        ...
                        if(snapshot.hasData){
                          ...
                          return ListView.builder(
                            ...
                            itemBuilder: (context, index){
                              ...
                              return DateFormat('MMMM yyyy').format(snap[index]['date'].toDate()) == _month ?
                              Container(
                                height: 90,
